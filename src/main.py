...
MIN_CREDIT_PERCENTAGE = float(os.getenv("MIN_CREDIT_PERCENTAGE", "0.15"))  # allow firmer credit on wider spreads
OI_THRESHOLD = int(os.getenv("OI_THRESHOLD", "100"))                   # lower OI threshold to include deeper strikes
SHORT_PUT_DELTA_RANGE = (-0.6, -0.4)  # deeper OTM for short leg
LONG_PUT_DELTA_RANGE = (-0.4, -0.2)   # deeper OTM for long leg
STRIKE_RANGE = float(os.getenv("STRIKE_RANGE", "0.30"))  # ±30% from spot for strike scan
